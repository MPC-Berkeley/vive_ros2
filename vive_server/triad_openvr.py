...
class TrackedDevice:
    ...
    def controller_state_to_dict(self, pControllerState):
        ...
        # https://gist.github.com/awesomebytes/75daab3adb62b331f21ecf3a03b3ab46 docs:
                ...
                if poses[i].bDeviceIsConnected:
                    ...
                    for device in config['devices']:
                        if device_serial == device['serial']:
                            ...
                            self.devices[device_name] = TrackedDevice(self.vr, i, device['type'])
        else:
            # Iterate through the pose list to find the active devices and determine their type
            for i in range(openvr.k_unMaxTrackedDeviceCount):
                if poses[i].bDeviceIsConnected:
    ...
    def __str__(self):
        ...
        for device_type in self.object_names:
            ...
            if len(self.object_names[device_type]) != 1:
                plural += "s"
            description += "Found " + str(len(self.object_names[device_type])) + " " + plural + "\n"
            for device in self.object_names[device_type]:
                if device_type == "Tracking Reference":
                    description += "  " + device + " (" + self.devices[device].get_serial() + ", Mode " + \
                                   self.devices[device].get_model() + ", " + self.devices[device].get_model() + ")\n"
                else:
                    description += "  " + device + " (" + self.devices[device].get_serial() + ", " + \
